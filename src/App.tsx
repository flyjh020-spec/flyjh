...
const App = () => {
  ...
  const [seats, setSeats] = useState(getInitialSeats('6x6'));
  ...
  const [seatHeight, setSeatHeight] = useState(40);
  ...
  const handleSeatKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const cols = 6;
      ...
      if (nextIndex >= seats.length) {
        ...
        if (currentCol + 1 < cols) {
          ...
          // Reached the last column's bottom
          return;
        }
      }
      if (nextIndex < seats.length) {
        const nextInput = document.getElementById(`seat-${nextIndex}`);
        if (nextInput) {
          nextInput.focus();
          setTimeout(() => (nextInput as HTMLInputElement).select(), 0);
        }
      }
    }
  };
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <div className="flex flex-col lg:flex-row gap-4 flex-1">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-2">
            <div className="bg-[#F5F4F0]/95 rounded-xl border border-[#E0DCD3] p-4 flex flex-col shadow-lg backdrop-blur-sm h-full">
              <div className="grid grid-cols-12 gap-1 text-center text-[#8B9DA8] font-bold mb-2 pb-1 border-b border-[#E0DCD3] text-[24px]">
              ...
              <div className="flex flex-col gap-1 flex-1">
                {schedule.map((item) => {
                  ...
                  return item.isBreak ? (
                    <div key={item.id} className={`grid grid-cols-12 gap-1 items-center text-center py-1 rounded-lg transition-all duration-300 ${isCurrent ? 'bg-[#6B8076] text-white shadow-md transform scale-[1.02]' : ''}`}>
                      <div className={`col-span-2 font-bold text-[24px] ${isCurrent ? 'text-white' : 'text-[#8C9C92]'}`}>{item.period}</div>
                      <div className={`col-span-4 tracking-widest font-bold text-[24px] ${isCurrent ? 'text-white' : 'text-[#A8A39D]'}`}>{item.time}</div>
                      <div className={`col-span-3 font-bold text-[24px] ${isCurrent ? 'text-white' : 'text-[#A8A39D]'}`}>{item.subject}</div>
                      <div className="col-span-3"></div>
                    </div>
                  ) : (
                    <div key={item.id} className={`grid grid-cols-12 gap-1 items-center text-center p-1 rounded-lg transition-all duration-300 ${isCurrent ? 'bg-[#6B8076] text-white shadow-md transform scale-[1.02] border border-[#5C5652]/30 z-10' : 'hover:bg-[#EAE7E0]/50'}`}>
                      <div className={`col-span-2 font-bold text-[24px] ${isCurrent ? 'text-white' : 'text-[#8C9C92]'}`}>{item.period}</div>
                      <div className="col-span-4">
                        <input type="text" value={item.time} onChange={(e) => updateSchedule(item.id, 'time', e.target.value)} className={`bg-transparent border-none text-center text-[24px] font-bold font-mono w-full outline-none rounded ${isCurrent ? 'text-white' : 'text-[#5C5652]'}`} />
                      </div>
                      <div className="col-span-3">
                        <div className={`relative rounded-md ${isCurrent ? 'border-2 border-white/50 bg-[#5C5652]/20' : 'bg-[#EAE7E0]/80 border border-[#DCD8CF]'}`}>
                          <select value={item.subject} onChange={(e) => updateSchedule(item.id, 'subject', e.target.value)} className={`w-full bg-transparent border-none py-1 px-1 text-center appearance-none outline-none cursor-pointer text-[24px] font-bold ${isCurrent ? 'text-white' : 'text-[#6B8076]'}`}>
                      ...
                      <div className="col-span-2 px-1">
                        <input type="text" value={item.bubbleCode || ''} onChange={(e) => updateSchedule(item.id, 'bubbleCode', e.target.value.replace(/\D/g, ''))} disabled={!item.isBubble} maxLength={3} className={`w-full text-center text-[24px] font-bold outline-none rounded-md py-0.5 ${!item.isBubble ? 'opacity-0' : isCurrent ? 'bg-white/20 text-white' : 'bg-white/50 text-[#6B8076] border border-[#DCD8CF]'}`} />
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            <div className="bg-[#F5F4F0]/95 rounded-xl border border-[#E0DCD3] p-6 flex flex-col relative shadow-lg backdrop-blur-sm transition-all duration-300 h-full">
                ...
                {seats.map((seat, index) => (
                  <div key={index} className="relative transition-all duration-200" style={{ height: `${seatHeight}px` }}>
                    <input 
                      ...
                      onKeyDown={(e) => handleSeatKeyDown(e, index)}
                      className="w-full h-full bg-white border border-[#DCD8CF] shadow-sm rounded-lg text-center text-2xl font-semibold text-[#6B8076] focus:border-[#8C9C92] outline-none" 
                    />
                  </div>
                ))}
              </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            <div className="bg-[#F5F4F0]/95 rounded-xl border border-[#E0DCD3] p-4 shadow-lg backdrop-blur-sm h-full flex flex-col gap-3 min-h-[180px]">
              <div className="grid grid-cols-3 gap-3 text-center">
                {Object.keys(attendance).filter(k => k !== 'notes').map(field => (
                  <div key={field} className="flex flex-col bg-white rounded-lg p-2 border border-[#DCD8CF] justify-center">
                    <span className="text-[#8B9DA8] text-sm mb-1">{field === 'expected' ? '應到' : field === 'present' ? '實到' : '缺考'}</span>
                    <input type="text" value={(attendance as any)[field]} onChange={(e) => setAttendance({...attendance, [field]: e.target.value})} className="bg-transparent text-center text-3xl font-bold text-[#5C5652] outline-none" />
                  </div>
                ))}
              </div>
              <div className="flex flex-col bg-white rounded-lg p-3 border border-[#DCD8CF] text-left flex-1">
                <span className="text-[#8B9DA8] text-sm font-bold mb-1">備註</span>
                <textarea value={attendance.notes} onChange={(e) => setAttendance({...attendance, notes: e.target.value})} className="bg-transparent text-[#5C5652] outline-none w-full text-base resize-none flex-1" />
              </div>
            </div>
          </div>
          ...
          {/* 右側：提醒事項 */}
          <div className="w-full lg:w-1/2 flex flex-col">
            <div className="bg-[#F5F4F0]/95 rounded-xl border border-[#E0DCD3] p-4 flex flex-col flex-1 shadow-lg backdrop-blur-sm min-h-[180px]">
